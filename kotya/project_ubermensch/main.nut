...
::iChapter03_Save_Portals <- [];

::Chapter_Init <- false;
::Chapter_Init_Time <- 2.0;

SKIP_PROLOGUE_VOTES <- [];
SKIP_PROLOGUE_PROCCENT <- 75;
...
g_iWarmUpTime <- 10
function Tick_WarmUp()
{
	local iProccent = (SKIP_PROLOGUE_VOTES.len() * 1.0 / GetPlayersArray().len() * 100).tointeger();
	if (iProccent < 0)
	{
		iProccent = 0;
	}
	...
	if (g_iWarmUpTime <= 0)
	{
		if (iProccent >= SKIP_PROLOGUE_PROCCENT)
		{
			iChapter = 1;
		}
		else
	...
	local text = Translate["skip_prologue"] + "\n \n"
	text += Translate["skip_prologue_status"] + " : <font color='#" + GetCOLORHEX(iProccent, SKIP_PROLOGUE_PROCCENT) +"'>" + iProccent + "</font>%/<font color='#00FF00'>" + SKIP_PROLOGUE_PROCCENT + "</font>%";
	ScriptPrintMessageCenterAll(text);
	PrintChatMessageAll(Translate["warmup"] + " : " + g_iWarmUpTime);
...
function Settings()
{
	...
	iChapter == 0)
	{
		CallFunction("ZombieToTeleport();", 237.05);
		SetSettings(0);
	}
	else if (iChapter == -1)
	{
...
//TICKS
{
	...
	const COLOR_TICKRATE = 4.00;
	::COLOR_TICKRATES <- 0.00;
	...
	::Tick_Color <- function()
	{
		...
		if (Time() < COLOR_TICKRATES)
		{
			return;
		}

		COLOR_TICKRATES = Time() + COLOR_TICKRATE;
...
//SELECT
{
	...
	::Enum_PickerType <- getconsttable().Enum_PickerType;
	...
	::RegSelect <- function()
	{
		...
		if (pickerID == -1)
		{
			printl("RegSelect pickerID ERROR");
			return;
		}
...
// TRIGGERS
{
	...
	::Enum_LIFT <- getconsttable().Enum_LIFT;
	...
	//SUBS
	{
		...
		function Text_CP4()
		{
			EF("map_entity_text", "SetText", Translate["date_cp4"]);
			EF("map_entity_text", "AddOutPut", "effect 2");
			EF("map_entity_text", "AddOutPut", "fadein 0.05");
			EF("map_entity_text", "AddOutPut", "y 0.50");
			EF("map_entity_text", "AddOutPut", "x 0.00");
		...
		function Text_CP5()
		{
			EF("map_entity_text", "SetText", Translate["date_cp5"]);
			EF("map_entity_text", "AddOutPut", "effect 2");
			EF("map_entity_text", "AddOutPut", "fadein 0.05");
			EF("map_entity_text", "AddOutPut", "y 0.50");
			EF("map_entity_text", "AddOutPut", "x 0.00");
	...
	function Trigger_Prologue_00()
	{
		...
		EF("map_trigger_start_human", "Disable", "", 104);
	...
	function Trigger_Prologue_03()
	{
		...
		EF("map_trigger_start_human", "Disable", "", 36);
	...
	function Spawn_Chapter02()
	{
		...
		EF("cp02", "ForceSpawn");
		EF("cp02_quest00_light01*", "TurnOff");
		EF("cp02_quest00_light02*", "TurnOff");
		EF("cp02_quest00_light03*", "TurnOff");
		EF("cp02_quest02_light00*", "TurnOff");
	...
	function Spawn_Location03()
	{
		...
		if (iChapter == 4)
		{
			EF("loc03_props_btr*", "Kill");
			EF("loc03_props_roadblock00*", "Kill");
		}
	}

	function Spawn_Chapter04()
	{
		...
		EF("map_trigger_start_human", "Enable", "");
		// EF("map_trigger_start_human", "Enable", "", 20);
		CallFunction("Trigger_Chapter4_01();", 1);
		...
		// CallFunction("StartOverLay(\"overlay/c4\", 7.0);", 20.0);

		CallFunction("Text_CP4();", 27);

		// CallFunction("Trigger_Chapter4_01();", 27);
	...
	function Spawn_Location04()
	{
		EF("loc04", "ForceSpawn");

		Location04_Spawn_MedKits();

	}

	function Spawn_Chapter05()
	{

	}

	function Spawn_Location05()
	{
		EF("loc05", "ForceSpawn");

		EF("loc05_props_ragdoll*", "DisableMotion", "", 3);
		Location05_Spawn_MedKits();
	}
	...
	::Trigger_Chapter1_00 <- function()
	{
		...
		GameTextInfo_Start(Translate["quest_z_debug"], Translate["quest_h_debug"], 15);
	...
	::Trigger_Chapter1_01 <- function()
	{
		...
		GameTextInfo_Start(Translate["quest_h_debug"], Translate["quest_z_debug"]);
	...
	::Trigger_Chapter1_03 <- function()
	{
		...
		foreach (player in PLAYERS)
		{
			...
			if (IsVectorInBoundingBox(player.GetOrigin(), Vector(1224, -124, -84), Vector(447.5, 892, 100)))
			{
				continue;
			}
	...
	//CP1_QUEST_01
	{
		::Trigger_Quest_01_Start <- function()
		{
			...
			local iRandomChance = 80;
			local iNearRadiusZombie = 512;
			local iNearRadiusHuman = 350;
		...
		::Trigger_Chapter1_Touch_Quest_02 <- function()
		{
			...
			if (TRIGGER_CHAPTER1_TOOLS == 1)
			{
				if (activator == cp01_quest_01.g_hOwner)
				{
					...
					EF("cp01_quest02_light*", "TurnOn", "", 2.0);
			...
			if (TRIGGER_CHAPTER1_TOOLS == 2)
			{
				...
				local Data = {
					...
					function Lose()
					{
						...
						EF("cp01_quest02_light*", "TurnOn", "", 3.0);
	...
	//CP1_LIFT
	{
		...
		::Lift_Button_CP01 <- function()
		{
			if (LIFT_INIT)
			{
				...
				EF("loc01_quest00_door*", "Close", "", 20.0);
				...
				PrintChatMessageAll(Enum_TextColor.Award + Translate["text19a"], 20);
				return;
			}
	...
	//CP1_QUEST_04
	{
		...
		::Trigger_Chapter1_08 <- function()
		{
			...
			foreach (player in PLAYERS)
			{
				...
				if (!IsVectorInBoundingBox(player.GetOrigin(), Vector(1920, -200, 1977), Vector(1152, 900, 500)))
				{
					Individual_Spore(player);
					continue;
				}
	...
	//CP2_QUEST
	{
		...
		::Trigger_Chapter2_01 <- function()
		{
			...
			EF("cp02_quest00_light02*", "TurnOn");
		...
		::Trigger_Chapter2_02 <- function()
		{
			...
			EF("cp02_quest00_light03*", "TurnOn");
		...
		::Trigger_Chapter2_03b <- function()
		{
			...
			EF("cp02_quest02_light00*", "TurnOn");
		...
		::Trigger_Chapter2_08 <- function()
		{
			...
			foreach (player in PLAYERS)
			{
				...
				if (!IsVectorInBoundingBox(player.GetOrigin(), Vector(6672, 1001, -24), Vector(368.5, 385, 160)))
				{
					Individual_Spore(player);
					continue;
				}
	...
	//CP3_QUEST
	{
		::Location03_Spawn_MedKits <- function()
		{
			local aVec = [
				...
				class_pos(Vector(8075, -1214, -152.46), Vector(0, 255, 0)),
				class_pos(Vector(6587, 258, -154), Vector(0, 165, 0)),
			];
		...
		::Trigger_Chapter3_07 <- function()
		{
			...
			CUT_SCENE_DATA = [
				{
					...
					a1 = Vector(60, 0, 0),
					time1 = 0.5,
					o2 = Vector(6952, 985, -75),
					a2 = Vector(28, -52, 0),
					time2 = 0.1,
					flytime = 6.0,
				},
				{
					o1 = Vector(6930, 1086, -143),
					a1 = Vector(22, -68, 0),
					time1 = 0.10,
					o2 = Vector(6930, 1086, -143),
					a2 = Vector(22, -68, 0),
					time2 = 0.10,
					flytime = 1.80,
				},
				{
					o1 = Vector(6127, 1003, 55),
					a1 = Vector(0, 0, 0),
					time1 = 0.50,
					o2 = Vector(6505, 1003, 55),
					a2 = Vector(30, 0, 0),
					time2 = 0.10,
					flytime = 15.00,
				},
				{
					o1 = Vector(6579, 972, -50),
					a1 = Vector(15, 0, 0),
					time1 = 1.3,
					o2 = Vector(6579, 758, -50),
					a2 = Vector(15, 0, 0),
					time2 = 0.10,
					flytime = 9.00,
				},
			...
			EF("cp03_quest05_sound*",  "PlaySound", "", 4.7);

			EF("map_entity_fade_end_chapter_in", "Fade");
			EF("map_entity_fade_end_chapter_out", "Fade", "", 2.5);

			EF("map_entity_fade_end_chapter_out", "Fade", "", 13.25);
			EF("map_entity_fade_end_chapter_in", "Fade", "", 25.5);
			EF("map_entity_fade_end_chapter_out", "Fade", "", 28.3);

			EF("map_entity_fade_end_chapter_in", "Fade", "", 35);
			EF("map_entity_fade_end_chapter_out", "Fade", "", 38);

			CallFunction("StartCutScene();", 4.65);

			CallFunction("Human_Glow_Disabled();");
			CallFunction("bFade = true;");

			EF("map_music_chapter_complete", "FireUser1", "", 42.5);

			CallFunction("Trigger_Chapter3_08();", 38.5);

			// EF("game_round_end", "EndRound_CounterTerroristsWin", "5.0", 12.0);

			PrintChatMessageAll(Enum_TextColor.Gold + Translate["missioncomplete"], 42.5);

			Chapter_End();
		}

		::Trigger_Chapter3_08 <- function()
		{
			EF("cp03_*", "Kill");
			EF("loc03_props_btr*", "Kill");
			EF("loc03_props_roadblock00*", "Kill");
		...
		// CallFunction("PlayMusic(7);", 4.0);

		// CallFunction("StartOverLay(\"overlay/c4\", 7.0);", 5.0);

			CallFunction("Spore_Start(300);", 5.0);
			CallFunction("Text_CP4();", 12);

			CallFunction("Trigger_Chapter4_01();", 12);

			CallFunction("Human_Glow_Enabled();", 12);
			CallFunction("bFade = false;", 10);
		}
	}

	//CP4_QUEST
	{
		::Location04_Spawn_MedKits <- function()
		{
			local aVec = [
				class_pos(Vector(13904, 4576, -145.299), Vector(-90, 90, 0)),
			];
		...
		::CH04_QUEST1_INIT <- false;
		::CH04_QUEST2_INIT <- false;
		::Trigger_Chapter4_01 <- function()
		{
			Portal_Scope.ZombieToTeleportRadius(Vector(7112, 666, -70), 1750);
			DISABLE_ABILITY = false;

			EF("loc03_quest00_door00*", "Open");
			EF("loc03_logic_main_door*", "Open");

			EF("cp04_logic_btr_button00*", "UnLock");
			CH04_QUEST1_INIT = false;
			CH04_QUEST2_INIT = false;
		}

		::Trigger_BTR_ON <- function()
		{
			if (CH04_QUEST1_INIT)
			{
				return
			}
			CH04_QUEST1_INIT = true;

			CallFunction("Trigger_Chapter4_02();", 5);
		}

		::Trigger_Chapter4_02 <- function()
		{
			BTR_SCRIPT.Start_Move();
		}

		::Trigger_Chapter4_03 <- function()
		{
			BTR_SCRIPT.Stop_Move();
		}

		::Trigger_Chapter4_04 <- function()
		{
			if (CH04_QUEST2_INIT)
			{
				return;
			}
			CH04_QUEST2_INIT = true;

			EF("cp04_quest01_collision*", "Kill");

			EF("cp04_effect00*", "Start");
			EF("cp04_sound00*", "PlaySound");

			EF("loc00_wall00*", "Toggle", "", 1.0);
			EF("loc03_wall00*", "FadeAndKill", "", 0.2);

			EF("cp04_effect00*", "Kill", "", 5);
			EF("cp04_sound00*", "Kill", "", 5);

			CallFunction("Trigger_Chapter4_05();", 3);
		}

		::Trigger_Chapter4_05 <- function()
		{
			BTR_SCRIPT.Start_Move();
		}

		::Trigger_Chapter4_06 <- function()
		{
			BTR_SCRIPT.Stop_Move();
			Portal_Scope.Portal_Init_CH04a();
			RemoveMedKitAtBoundBox(Vector(7754, -2162, 1359), Vector(3700, 3581.5, 2532.5));

			EF("loc00_wall00*", "Toggle");
			CallFunction("Trigger_Chapter4_07();", 60);
			...
			foreach (player in PLAYERS)
			{
				...
				if (player.GetTeam() != 3)
				{
					if (!IsVectorInBoundingBox(player.GetOrigin(), Vector(7754, -2162, 1359), Vector(3700, 3581.5, 2532.5)))
					{
						continue;
					}
					CreatePortalPickerClass(player);
		...
		::Trigger_Chapter4_07 <- function()
		{
			EF("loc05_logic_door00*", "Open");

			CallFunction("Trigger_Chapter4_08();", 5);
		}

		::Trigger_Chapter4_08 <- function()
		{
			BTR_SCRIPT.Start_Move();
		}

		::Trigger_Chapter4_09 <- function()
		{
			BTR_SCRIPT.Stop_Move();

			EF("loc05_logic_button00*", "UnLock");
		}

		::Trigger_Chapter4_09a <- function()
		{
			if (iChapter == 4)
			{
				EF("loc05_logic_button00*", "Lock");
				EF("loc05_logic_door01*", "Open");

				CallFunction("Trigger_Chapter4_10();", 7);
				return;
			}
		}

		::Trigger_Chapter4_10 <- function()
		{
			BTR_SCRIPT.Start_Move();
		}

		::Trigger_Chapter4_11 <- function()
		{
			BTR_SCRIPT.Stop_Move();
			...
			EF("loc05_logic_door01*", "Close", "", 4.5);
			EF("cp04_quest02_trigger_zm*", "Enable", "", 10);
			CallFunction("Trigger_Chapter4_12();", 10.05);
		}

		Trigger_Chapter4_12_I <- function()
		{
			END_ZOMBIE_INSIDE = true;
		}

		Trigger_Chapter4_12_O <- function()
		{
			EF(caller, "Kill");
		...
		::Trigger_Chapter4_12 <- function()
		{
			...
			foreach (player in PLAYERS)
			{
				...
				if (!IsVectorInBoundingBox(player.GetOrigin(), Vector(9919, 7481, 54), Vector(1671, 988, 258)))
				{
					Individual_Spore(player);
					continue;
				}
			...
			// CallFunction("StartOverLay(\"overlay/c5\", 7.0);", 3.5);
			CallFunction("Text_CP5();", 10.5);
	...
	//CP5_QUEST
	{
		::Location05_Spawn_MedKits <- function()
		{
			local aVec = [
				class_pos(Vector(10517, 6849, -163), Vector(0, 45, 0)),
				class_pos(Vector(10077, 8425, 53), Vector(0, 60, 0)),
				class_pos(Vector(9384, 7231, -162), Vector(0, 0, 0)),
				class_pos(Vector(10023, 9153, 76.6638), Vector(-90, 90, 0)),
			];
...
// GAMES
{
	...
	::Enum_MiniGame_End <- getconsttable().Enum_MiniGame_End;
...
//CUT_SCENE
{
	...
	{
		CUT_SCENE_MAKER_STARTTIME = Time();
	...
	::NextCameras <- function()
	{
		...
		printl(CUT_SCENE_ID + ")" + (Time() - CUT_SCENE_MAKER_STARTTIME));
...
//FREE CAMERA
{
	...
	::class_presonal_camera <- class
	{
		...
		constructor()
		{
			...
			this.camera.__KeyValueFromInt("wait", 99999);
			this.camera.__KeyValueFromString("targetname", "camera" + PERSONAL_CAMS.len());
		}
...
//TEXTCOLOR
{
	...
	::Enum_TextColor <- getconsttable().Enum_TextColor;
}
//CLASS_SETTINGS
{
	::SETTINGS <- [];
	...
	::Enum_SettingsValue <- getconsttable().Enum_SettingsValue;
...
//EVENT HOOKER
{
	...
	function Event_Start()
	{
		...
		// 	REG_EVENT(EVENT_LIST);
		// }
		if (GLOBAL_ZONE == null || GLOBAL_ZONE != null && !GLOBAL_ZONE.IsValid())
		{
			GLOBAL_ZONE = Entities.FindByName(null, "map_eventlistener_zone");
		}
		// if (EVENT_INFO == null || EVENT_INFO != null && !EVENT_INFO.IsValid())
		// {
		// 	EVENT_INFO = Entities.FindByName(null, "map_eventlistener_player_info");
		// 	REG_EVENT(EVENT_INFO);
		// }
		// if (EVENT_FALL == null || EVENT_FALL != null && !EVENT_FALL.IsValid())
		// {
		// 	EVENT_FALL = Entities.FindByName(null, "map_eventlistener_player_falldamage");
		// 	REG_EVENT(EVENT_FALL);
		// }
		// if (EVENT_CHAT == null || EVENT_CHAT != null && !EVENT_CHAT.IsValid())
		// {
		// 	EVENT_CHAT = Entities.FindByName(null, "map_eventlistener_player_chat");
		// 	REG_EVENT(EVENT_CHAT);
		// }
	...
	function PlayerConnect()
	{
		...
		local event_data = EVENT_LIST.GetScriptScope().eventqueue[0];
		EVENT_LIST.GetScriptScope().eventqueue.remove(0);

		local userid = event_data.userid;
		local name = event_data.name;
		local steamid = event_data.networkid;
	...
	function PlayerInfo()
	{
		...
		local event_data = EVENT_INFO.GetScriptScope().eventqueue[0];
		EVENT_INFO.GetScriptScope().eventqueue.remove(0);
	...
	function PlayerChat()
	{
		...
		local event_data = EVENT_CHAT.GetScriptScope().eventqueue[0];
		EVENT_CHAT.GetScriptScope().eventqueue.remove(0);
	...
	function PlayerFall()
	{
		...
		local event_data = EVENT_FALL.GetScriptScope().eventqueue[0];
		EVENT_FALL.GetScriptScope().eventqueue.remove(0);
		...
		local damage = event_data.damage;
	...
	{
		AOP(Entity_Maker, "EntityTemplate", "map_eventlistener");
	...
	::REG_EVENT <- function(event_handle)
	{
		if (event_handle == null ||
		!event_handle.IsValid())
		{
			return;
		}
		// event_handle.__KeyValueFromString("classname", "info_target");
		EntFireByHandle(event_handle, "AddOutput", "classname info_target", 0.00, null, null);
		event_handle.ValidateScriptScope();
		local sc = event_handle.GetScriptScope();
		sc.eventqueue <- [];
		if ("event_data" in sc)
		{
			sc.d <- function(){delete event_data};
			sc.d();
		}
		sc.eventdata <- sc.eventqueue.weakref();
		delegate {function _newslot(k, v){eventdata.push(v)}}:sc;
	}
}
//CHAT
{
...
//GAMETEXTINFO
{
	const GAMETEXTINFO_TICKRATE = 0.5;
	::GAMETEXTINFO_TICKRATES <- 0;

	::GAMETEXTINFO_BUFER <- [];
	::GAMETEXTINFO_TICKING <- false;
	::GAMETEXTINFO_SHOWLASTFRAME <- false;

	enum Enum_GameTextInfoType
	{
		Green,
		Blue,
	}
	::Enum_GameTextInfoType <- getconsttable().Enum_GameTextInfoType;

	::ResetGameTextInfo <- function()
	{
		GAMETEXTINFO_BUFER.clear();
		GAMETEXTINFO_TICKING = false;
		GAMETEXTINFO_SHOWLASTFRAME = false;
	}

	::GameTextInfo_Debug1 <- function()
	{
		GameTextInfo_Start(Translate["quest_h_debug"], Translate["quest_z_debug"], 1, Enum_GameTextInfoType.Green);
	}
	::GameTextInfo_Debug2 <- function()
	{
		GameTextInfo_Start(Translate["quest_z_debug"], Translate["quest_h_debug"], 0, Enum_GameTextInfoType.Blue);
	}

	::GameTextInfo_Start <- function(text1, text2, delay = 0.0, GameTextInfoType = Enum_GameTextInfoType.Green)
	{
		if (GameTextInfoType == Enum_GameTextInfoType.Green)
		{
			EF("map_entity_text_quest_h", "SetTextColor", "0 255 0");
		}
		else if (GameTextInfoType == Enum_GameTextInfoType.Blue)
		{
			EF("map_entity_text_quest_h", "SetTextColor", "0 255 255");
		}

		GAMETEXTINFO_BUFER.clear();
		GAMETEXTINFO_BUFER.push(text1);
		GAMETEXTINFO_BUFER.push(text2);

		if (GAMETEXTINFO_TICKING ||
		delay > 0.0)
		{
			if (delay <= 0.0)
			{
			...
			CallFunction("GameTextInfo_StartPost();", delay);
			return;
		}

		GameTextInfo_StartPost();
	}
	::GameTextInfo_StartPost <- function()
	{
		EF("map_entity_text_quest_z", "SetText", GAMETEXTINFO_BUFER[0]);
		EF("map_entity_text_quest_h", "SetText", GAMETEXTINFO_BUFER[1]);

		GAMETEXTINFO_TICKING = true;
	}

	::Tick_GameTextInfo <- function()
	{
		if (CUT_SCENE)
		{
			if (GAMETEXTINFO_SHOWLASTFRAME)
			{
				EF("map_entity_text_quest_reset", "Display");
			}
		...
		if (Time() < GAMETEXTINFO_TICKRATES)
		{
			return;
		}
		GAMETEXTINFO_TICKRATES = Time() + GAMETEXTINFO_TICKRATE;
		GAMETEXTINFO_SHOWLASTFRAME = true;

		foreach (player in PLAYERS)
		{
			if (GetPortalPickerClassByOwner(player) != null ||
			GetPickerClassByOwner(player) != null)
			{
				continue;
			}
			...
			if (player.GetTeam() == 2)
			{
				EntFire("map_entity_text_quest_z", "Display", "", 0.00, player);
			}
			else
			{
				EntFire("map_entity_text_quest_h", "Display", "", 0.00, player);
			}
		}
	}
}
...
function Init()
{
	...
	Entity_Maker = Entities.CreateByClassname("env_entity_maker");
	EntFireByHandle(Entity_Maker, "AddOutput", "classname info_target", 0.00, null, null);
	Init_Events();
	Init_Settings();
	EF("skycamera_main", "ActivateSkyBox");
...
function Hook_SkipPrologue()
{
	foreach (_activator in SKIP_PROLOGUE_VOTES)
	{
	...
	SKIP_PROLOGUE_VOTES.push(activator);
}
