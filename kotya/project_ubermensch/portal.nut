...
::PORTAL_HP <- 7000;
::PORTAL_ZOMBIE_QUEST_HP <- 7000;

::PORTAL_LIMIT <- 5;
::PORTAL_CD <- 15.0;

::PORTAL_ZOMBIE_QUEST_LIMIT <- 10;
::PORTAL_ZOMBIE_QUEST_CD <- 15.0;
...
::Portal_Init_CH01 <- function()
{
	...
	CreatePortal(aData);
	iChapter01_Save_Portals.push([aData, PORTALS.len() - 1]);

	_portal_pos = class_pos(Vector(3072, -672, -195), Vector(0, -1, 0));
	_cam_pos = class_pos(Vector(3357, -1219, -87), Vector(2, 107, 0));

	aData = {portal_pos = null, cam_pos = null, allow_berserk = true, allow_vulture = true, allow_tank = true, disable = false, limited = true, limit_count = PORTAL_LIMIT, limit_cd = PORTAL_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
...
::Portal_Init_CH01a_Quest_Zombie <- function()
{
	...
	aData = {portal_pos = null, cam_pos = null, allow_berserk = true, allow_vulture = true, allow_tank = true, disable = true, limited = true, limit_count = PORTAL_ZOMBIE_QUEST_LIMIT, limit_cd = PORTAL_ZOMBIE_QUEST_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
	...
	CreatePortal(aData);
	TEMP_PORTAL.Enable();
	TEMP_PORTAL.SetHealth(PORTAL_ZOMBIE_QUEST_HP);

	aData  = {portal_pos = null, cam_pos = null, allow_berserk = true, allow_vulture = true, allow_tank = true, disable = false, limited = true, limit_count = PORTAL_ZOMBIE_QUEST_LIMIT, limit_cd = PORTAL_ZOMBIE_QUEST_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
	aData.portal_pos = _portal_pos;
	aData.cam_pos = _cam_pos;
...
::Portal_Init_CH03a_Quest_Zombie <- function()
{
	...
	iChapter03_Save_Portals.push([aData, PORTALS.len() - 1]);
}

::Portal_Init_CH05a_Quest_Zombie <- function()
{
	local aData;
	local _portal_pos;
	local _cam_pos;

	_portal_pos = class_pos(Vector(9178, 7526, -203), Vector(-1, 0, 0));
	_cam_pos = class_pos(Vector(9507, 6473, 177), Vector(16, 108, 0));
...
::Portal_Init_CH01_Quest01 <- function()
{
	...
	aData = {portal_pos = null, cam_pos = null, allow_berserk = true, allow_vulture = true, allow_tank = true, disable = true, limited = true, limit_count = PORTAL_LIMIT, limit_cd = PORTAL_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
...
::Portal_Init_CH01_Quest04 <- function()
{
	...
	CreatePortal(aData);
	TEMP_PORTAL.Enable();
	aData  = {portal_pos = null, cam_pos = null, allow_berserk = false, allow_vulture = true, allow_tank = false, disable = false, limited = true, limit_count = PORTAL_LIMIT, limit_cd = PORTAL_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
	aData.portal_pos = _portal_pos;
	aData.cam_pos = _cam_pos;
...
::Portal_Init_CH03_Save <- function()
{
	if (iChapter03_Save_Portals.len() < 1)
	{
		return;
	}

	foreach (saveportal in iChapter03_Save_Portals)
	{
		CreatePortal(saveportal[0]);
	}
}
...
::Portal_Init_CH03a <- function()
{
	...
	aData = {portal_pos = null, cam_pos = null, allow_berserk = true, allow_vulture = false, allow_tank = true, disable = false, limited = true, limit_count = PORTAL_LIMIT, limit_cd = PORTAL_CD, kv_hitbox = {}, kv_trigger = {}, kv_model = {}};
...
::Portal_Init_CH04 <- function()
{
	...
	_portal_pos = class_pos(Vector(12965, 3143, 165), Vector(1, 0, 0));
	_cam_pos = class_pos(Vector(12101, 3858, 316), Vector(23, -2, 0));
	...
	_portal_pos = class_pos(Vector(12803, 1532, -203), Vector(0, 1, 0));
	_cam_pos = class_pos(Vector(12380, 2099, 355), Vector(41, -84, 0));
	...
	CreatePortal(aData);
	TEMP_PORTAL.SetHealth(PORTAL_HP * 2);

	_portal_pos = class_pos(Vector(13588, 6426, -203), Vector(-1, 0, 0));
	_cam_pos = class_pos(Vector(14131, 6387, -29), Vector(8, -132, 0));
	...
	_portal_pos = class_pos(Vector(11181, 3016, -203), Vector(1, 0, 0));
	_cam_pos = class_pos(Vector(10988, 3617, 53), Vector(10, -24, 0));
...
::Portal_Init_CH04a <- function()
{
	foreach (portal in PORTALS)
	{
		if (IsVectorInBoundingBox(portal.GetSelfOrigin(), Vector(7754, -2162, 1359), Vector(3700, 3581.5, 2532.5)))
		{
			portal.Death(false);
		}
	}
...
::Portal_Init_CH05 <- function()
{
	local _portal_pos;
	local _cam_pos;
	local aData;

	_portal_pos = class_pos(Vector(12956, 7362, -203), Vector(0, 1, 0));
	_cam_pos = class_pos(Vector(12944, 7055, -88), Vector(-4, 141, 0));

	aData = {portal_pos = null, cam_pos = null, allow_berserk = false, allow_vulture = true, allow_tank = false, disable = false, limited = false, limit_count = -1, limit_cd = -1.0, kv_hitbox = null, kv_trigger = {}, kv_model = {}};
	...
	_portal_pos = class_pos(Vector(10555, 4683, -202), Vector(0, 1, 0));
	_cam_pos = class_pos(Vector(10272, 4495, -47), Vector(2, 80, 0));
	...
	_portal_pos = class_pos(Vector(7195, 6655, -202), Vector(0, -1, 0));
	_cam_pos = class_pos(Vector(7204, 6296, -28), Vector(5, 37, 0));
	...
	_portal_pos = class_pos(Vector(8532, 5375, -50), Vector(1, 0, 0));
	_cam_pos = class_pos(Vector(8295, 5394, 229), Vector(23, 38, 0));
...
//PORTAL_DATA
{
	...
	::RemovePortal <- function(ID)
	{
		...
		else if (iChapter == 3)
		{
			...
			foreach (index, portal in iChapter03_Save_Portals)
			{
				printl(portal[1] + " : " + ID);
				if (portal[1] == ID)
				{
					iChapter03_Save_Portals.remove(index);
					return;
				}
			}
		}
...
//
	...
	function Portal_Maker_CH05a()
	{
		local Data = {
			function Complete()
			{
				Portal_Scope.Portal_Init_CH05a_Quest_Zombie();
			},
		}

		CreatePortal_Maker(Vector(9171, 7389, -190), Data);
	}
}