...
function Tick()
{
	...
	else
	{
		if(canmove)
		{
			...
			ttime+=TICKRATE;
			if(TraceLine(HitBox.GetOrigin(),target.GetOrigin()+Vector(0,0,10),self)!=1.00)counter+=0.10;
...
function SearchTarget()
{
	...
	while(null!=(h=Entities.FindInSphere(h,self.GetOrigin(),TARGET_DISTANCE)))
	{
		...
		{
			counterCT++;
			if(TraceLine(HitBox.GetOrigin(),h.GetOrigin()+Vector(0,0,20),self)==1.00)
			{
				TestLine(HitBox.GetOrigin(),h.GetOrigin()+Vector(0,0,20));
				candidates.push(h);
			}
			else TestLineRed(HitBox.GetOrigin(),h.GetOrigin()+Vector(0,0,20));
		}
	}
...
function TestLine(Start,End)DebugDrawLine(Start,End, 255, 255, 255, true, 2)
function TestLineRed(Start,End)DebugDrawLine(Start,End, 255, 0, 0, true, 2)
