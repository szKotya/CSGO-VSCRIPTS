...
{
    local h = null;
	while(null != (h = Entities.FindInSphere(h,caller.GetOrigin(),200)))
    {
        if(h.GetName().find("item_rocket_launcher_model") == 0)
        {
            local id = (h.GetName().slice(h.GetPreTemplateName().len(),h.GetName().len())).tostring();
            EntFire("item_rocket_launcher_ui"+id, "RunScriptCode", "SetRocket();", 0.0, caller)
            return
        }
    }
}
