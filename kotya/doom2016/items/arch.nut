...
function SetHandle()
{
    ...
    if(clas == "func_physbox_multiplayer"){Hbox = caller;caller.SetOwner(self);}
    if(clas == "func_door" && name.find("item_arch_parent") == 0)Parent = caller;
    if(clas == "func_door" && name.find("item_arch_eye") == 0)Eye = caller;
    if(clas == "ambient_generic" && name.find("item_arch_sound_attack") == 0)SoundAttack = caller;
    if(clas == "ambient_generic" && name.find("item_arch_sound") == 0)Sound = caller;
    if(clas == "logic_measure_movement" && name.find("item_arch_measure") == 0)Measure = caller;
    if(clas == "logic_measure_movement" && name.find("item_arch_measure_eye") == 0)MeasureEye = caller;
    if(clas == "prop_dynamic_glow")Model = caller;
    if(clas == "trigger_hurt")ArmHurt = caller;
    if(clas == "env_entity_maker" && name.find("item_arch_fireball_maker") == 0)FireBall = caller;
    if(clas == "env_entity_maker" && name.find("item_arch_firewave_maker") == 0)FireWave = caller;
    if(clas == "env_entity_maker" && name.find("item_arch_firelake") == 0)FireLake = caller;
}
