...
function Help() 
{
    ...
    text = "\x09" + Command_pref + "transfer <userid> <amount>\x01 - transfers your\x04 Money\x01 to the player";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x09" + Command_pref + "driver\x01 -\x04 enables\x01/" + "\x02" + "disables\x01 the opportunity for others to pickup your\x04 Waffel Car";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x01[\x04VIP\x01]\x09" + Command_pref + "waffelcar\x01 - get a\x04 Waffel Car";
    ScriptPrintMessageChatAll(Chat_pref + text);
...
function ToggleWaffel(userid)
{
    ...
    if(handle == null)
        return;
    if(!handle.IsValid() && handle.GetHealth() <= 0 && handle.GetTeam() == 1)
        return;
    ...
    if(car_class == null)
        return;
    waffel_car.GetScriptScope().Toggle(handle);
}
