...
::WAFFEL_CAR_ENABLE <- false; // set false to disable this
...
function MapStart()
{
	...
	if(fog == null || fog != null && !fog.IsValid())
    {
        fog = Entities.FindByClassname(null, "env_fog_controller");
    }
	if(skycamera == null || skycamera != null && !skycamera.IsValid())
    {
        skycamera = Entities.FindByClassname(null, "sky_camera");
    }
	if(tone == null || tone != null && !tone.IsValid())
    {
        tone = Entities.FindByClassname(null, "env_tonemap_controller");
    }
...
"STEAM_1:1:159416248",	//toppi make skin for VIP
...
function GetItem(item_temp, handle)
{
	...
	EntFireByHandle(self, "RunScriptCode", "activator.SetOrigin(Vector(" + (makerpos.x) + "," + (makerpos.y) + "," + (makerpos.z - 30) + "));", 0, handle, handle);
...
function PickUpItem()
{
	...
	if(name.find("bio") != null)
	{
		...
		if(pl.item_buff_radius)
		{
			...
			text += " + "+ (item.GetRadius(lvl) * 0.1).tostring();
		}
	...
	ScriptPrintMessageChatAll(item_name+"pickup \x04"+pl.name+"\x01 {\x07"+pl.steamid+"\x01} "+color+""+lvl+"\x01 level ***")
					...
					if(pl.GetHealth() >= 600 && pl.GetTeam() == 2)
					{
						if(PLAYERS[i].setPerks == false)
						{
							...
							if(PLAYERS[i].perkchameleon_lvl > 0)
							{
								...
								pl.__KeyValueFromInt("renderamt", 175 - PLAYERS[i].perkchameleon_lvl * perkchameleon_chameleonperlvl);
							}
...
function ToggleParticles()
{
	...
	EntFire("Bar_Fire", "StartFire", "", 1.00, null);
}
...
function Trigger_CosmoBar()
{
	...
	EntFire("rope_ct_conyon", "RunScriptCode", "SelfBreak()", 39.5);
	EntFire("rope_ct_city1", "RunScriptCode", "SelfBreak()", 39.5);
	EntFire("rope_ct_spawn", "RunScriptCode", "SelfBreak()", 39.5);
	EntFire("rope_ct_city", "RunScriptCode", "SelfBreak()", 39.5);
...
function Trigger_Rock()
{
	...
	EntFire("rope_ct_tree", "RunScriptCode", "Start()", 25.05 + timer + 5.00);
	EntFire("rope_light_tree", "ShowSprite", "", 25.05 + timer + 5.00);
	...
	EntFire("rope_ct_bar", "RunScriptCode", "SelfBreak()", 0.5);
	EntFire("rope_ct_afterbar", "RunScriptCode", "SelfBreak()", 0.5);
}
...
function Trigger_Cave_First()
{
	...
	EntFire("Temp_Cave", "ForceSpawn", "", 20);
	if(Stage == 1)
...
function Trigger_Cave_Second()
{
	...
	EntFire("Bar_Prop", "Kill", "", 0);
	EntFire("bar_Fire", "Kill", "", 0);

	EntFire("Bar_Steam", "Kill", "", 0);
	EntFire("Cosmo_Bar_Door", "Kill", "", 0);
	EntFire("lvl3_Wood_Door", "Kill", "", 0);
	EntFire("kojima_trigger_02", "Kill", "", 0);
	
	EntFire("rope_ct_tree", "RunScriptCode", "SelfBreak()", 0);
...
function Trigger_After_Boss_Skip_First() 
{
	...
	EntFire("Boss_cage_skip", "Kill", "", time + 0.01);
}
...
function SetInvalid(handle)
{
	...
	local makerpos = GetSpot();
	invalid_maker.SetOrigin(makerpos);

	EntFireByHandle(invalid_maker, "ForceSpawn", "", 0, null, null);
	EntFireByHandle(self, "RunScriptCode", "activator.SetOrigin(Vector(" + (makerpos.x) + "," + (makerpos.y) + "," + (makerpos.z - 30) + "));", 0, handle, handle);
	EntFireByHandle(self, "RunScriptCode", "activator.SetOrigin(Vector(" + (handlepos.x) + "," + (handlepos.y) + "," + (handlepos.z) + "));", 0.05, handle, handle);
}
...
obj = Item("fire","Creates a fire zone that burns ZMs","360 512 600","5 6 7","80 75 70","500 600 700","0.15 0.2 0.25");
...
obj = Item("earth","Creates a breakable rock wall" ,"360 512 600","6 7 8","80 75 70","1000 2000 3000","0.5 0.5 0.5");
